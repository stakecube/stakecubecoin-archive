...
bool CMasternodePaymentDB::Write(const CMasternodePayments& objToSave)
{
    ...
    LogPrint("mnpayments","Written info to mnpayments.dat  %dms\n", GetTimeMillis() - nStart);
...
CMasternodePaymentDB::ReadResult CMasternodePaymentDB::Read(CMasternodePayments& objToLoad, bool fDryRun)
{
    ...
    LogPrint("mnpayments","Loaded info from mnpayments.dat  %dms\n", GetTimeMillis() - nStart);
    LogPrint("mnpayments","  %s\n", objToLoad.ToString());
    if (!fDryRun) {
        LogPrint("mnpayments","Masternode payments manager - cleaning....\n");
        objToLoad.CleanPaymentList();
        LogPrint("mnpayments","Masternode payments manager - result:\n");
        LogPrint("mnpayments","  %s\n", objToLoad.ToString());
    }
...
void DumpMasternodePayments()
{
    ...
    LogPrint("mnpayments","Verifying mnpayments.dat format...\n");
    ...
    if (readResult == CMasternodePaymentDB::FileError)
        LogPrint("mnpayments","Missing budgets file - mnpayments.dat, will try to recreate\n");
    else if (readResult != CMasternodePaymentDB::Ok) {
        LogPrint("mnpayments","Error reading mnpayments.dat: ");
        if (readResult == CMasternodePaymentDB::IncorrectFormat)
            LogPrint("mnpayments","magic is ok but data has invalid format, will try to recreate\n");
        else {
            LogPrint("mnpayments","file format is unknown or invalid, please fix it manually\n");
            return;
        }
    }
    LogPrint("mnpayments","Writting info to mnpayments.dat...\n");
    paymentdb.Write(masternodePayments);

    LogPrint("mnpayments","Budget dump finished  %dms\n", GetTimeMillis() - nStart);
}
...
bool IsBlockValueValid(const CBlock& block, CAmount nExpectedValue, CAmount nMinted)
{
    ...
    if (nHeight == 0) {
        LogPrint("mnpayments","IsBlockValueValid() : WARNING: Couldn't find previous block\n");
    }
...
bool IsBlockPayeeValid(const CBlock& block, int nBlockHeight)
{
    ...
    if (IsSporkActive(SPORK_11_ENABLE_SUPERBLOCKS)) {
        if (budget.IsBudgetPaymentBlock(nBlockHeight)) {
            ...
            if (transactionStatus == TrxValidationStatus::InValid) {
                LogPrint("mnpayments","Invalid budget payment detected %s\n", txNew.ToString().c_str());
    ...
    if (masternodePayments.IsTransactionValid(txNew, nBlockHeight))
        return true;
    LogPrint("mnpayments","Invalid mn payment detected %s\n", txNew.ToString().c_str());
...
void FillBlockPayee(CMutableTransaction& txNew, CAmount nFees, bool fProofOfStake)
{
    ...
    if (IsSporkActive(SPORK_11_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(pindexPrev->nHeight + 1)) {
...
{
    if (IsSporkActive(SPORK_11_ENABLE_SUPERBLOCKS) && budget.IsBudgetPaymentBlock(nBlockHeight)) {
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    if (!masternodePayments.GetBlockPayee(pindexPrev->nHeight + 1, payee)) {
        ...
        if (winningNode) {
            ...
            LogPrint("mnpayments","CreateNewBlock: Failed to detect masternode to pay\n");
            hasPayment = false;
        }
    }
...
void CMasternodePayments::ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == NetMsgType::MNGET) { //Masternode Payments Request Sync
        ...
        if (!masternodePayments.CanVote(winner.vinMasternode.prevout, winner.nBlockHeight)) {
            //  LogPrint("mnpayments","mnw - masternode already voted - %s\n", winner.vinMasternode.prevout.ToStringShort());
            return;
        }
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount blockValue = GetBlockValue(pindexPrev->nHeight+1);
    CAmount requiredMasternodePayment = GetMasternodePayment(pindexPrev->nHeight+1, blockValue);
    ...
    LogPrint("mnpayments","CMasternodePayments::IsTransactionValid - Missing required payment of %s to %s\n", FormatMoney(requiredMasternodePayment).c_str(), strPayeesPossible.c_str());
    return false;
}
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    if (budget.IsBudgetPaymentBlock(nBlockHeight)) {
        ...
        if (pmn != NULL) {
            LogPrint("mnpayments","CMasternodePayments::ProcessBlock() Found by FindOldestNotInVec \n");
            ...
            LogPrint("mnpayments","CMasternodePayments::ProcessBlock() Winner payee %s nHeight %d. \n", EncodeDestination(address1).c_str(), newWinner.nBlockHeight);
        } else {
            LogPrint("mnpayments","CMasternodePayments::ProcessBlock() Failed to find masternode to pay\n");
        }
    }
    ...
    if (!CMessageSigner::GetKeysFromSecret(strMasterNodePrivKey, keyMasternode, pubKeyMasternode)) {
        LogPrint("mnpayments","CMasternodePayments::ProcessBlock() - Error upon calling GetKeysFromSecret.\n");
        return false;
    }

    LogPrint("mnpayments","CMasternodePayments::ProcessBlock() - Signing Winner\n");
    if (newWinner.Sign(keyMasternode, pubKeyMasternode)) {
        LogPrint("mnpayments","CMasternodePayments::ProcessBlock() - AddWinningMasternode\n");
