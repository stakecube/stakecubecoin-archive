...
//
// Bootup the Masternode, look for a 1000 SCC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        CNode* pnode = ConnectNode(CAddress(service, NODE_NETWORK), NULL);
        if (!pnode) {
...
std::vector<COutput> CActiveMasternode::SelectCoinsMasternode()
{
    ...
    for (const COutput& out : vCoins) {
        if (out.tx->vout[out.i].nValue == Params().RequiredMasternodeCollateral() * COIN) { //exactly
