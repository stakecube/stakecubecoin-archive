...
void ScriptPubKeyToJSON(const CScript& scriptPubKey, UniValue& out, bool fIncludeHex)
{
    ...
    out.push_back(make_pair("asm", scriptPubKey.ToString()));
    if (fIncludeHex)
        out.push_back(make_pair("hex", HexStr(scriptPubKey.begin(), scriptPubKey.end())));

    if (!ExtractDestinations(scriptPubKey, type, addresses, nRequired)) {
        out.push_back(make_pair("type", GetTxnOutputType(type)));
        return;
    }

    out.push_back(make_pair("reqSigs", nRequired));
    out.push_back(make_pair("type", GetTxnOutputType(type)));

    UniValue a(UniValue::VARR);
    BOOST_FOREACH (const CTxDestination& addr, addresses)
        a.push_back(EncodeDestination(addr));
    out.push_back(make_pair("addresses", a));
}

void TxToJSON(const CTransaction& tx, const uint256 hashBlock, UniValue& entry, bool include_hex, int serialize_flags)
{
    entry.push_back(make_pair("txid", tx.GetHash().GetHex()));
    entry.push_back(make_pair("hash", tx.GetWitnessHash().GetHex()));
    entry.push_back(make_pair("size", (int)::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION)));
    entry.push_back(make_pair("vsize", (int)::GetVirtualTransactionSize(tx)));
    entry.push_back(make_pair("version", tx.nVersion));
    entry.push_back(make_pair("locktime", (int64_t)tx.nLockTime));
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (tx.IsCoinBase())
            in.push_back(make_pair("coinbase", HexStr(txin.scriptSig.begin(), txin.scriptSig.end())));
        else {
            in.push_back(make_pair("txid", txin.prevout.hash.GetHex()));
            in.push_back(make_pair("vout", (int64_t)txin.prevout.n));
            UniValue o(UniValue::VOBJ);
            o.push_back(make_pair("asm", txin.scriptSig.ToString()));
            o.push_back(make_pair("hex", HexStr(txin.scriptSig.begin(), txin.scriptSig.end())));
            in.push_back(make_pair("scriptSig", o));
        }
        if (!tx.wit.IsNull()) {
            if (!tx.wit.vtxinwit[i].IsNull()) {
                ...
                in.push_back(make_pair("txinwitness", txinwitness));
            }

        }
        in.push_back(make_pair("sequence", (int64_t)txin.nSequence));
        vin.push_back(in);
    }
    entry.push_back(make_pair("vin", vin));
    UniValue vout(UniValue::VARR);
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        const CTxOut& txout = tx.vout[i];
        UniValue out(UniValue::VOBJ);
        out.push_back(make_pair("value", ValueFromAmount(txout.nValue)));
        out.push_back(make_pair("n", (int64_t)i));
        UniValue o(UniValue::VOBJ);
        ScriptPubKeyToJSON(txout.scriptPubKey, o, true);
        out.push_back(make_pair("scriptPubKey", o));
        vout.push_back(out);
    }
    entry.push_back(make_pair("vout", vout));

    if (!hashBlock.IsNull()) {
        entry.push_back(make_pair("blockhash", hashBlock.GetHex()));
        BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pindex = (*mi).second;
            if (chainActive.Contains(pindex)) {
                entry.push_back(make_pair("confirmations", 1 + chainActive.Height() - pindex->nHeight));
                entry.push_back(make_pair("time", pindex->GetBlockTime()));
                entry.push_back(make_pair("blocktime", pindex->GetBlockTime()));
            } else
                entry.push_back(make_pair("confirmations", 0));
        }
    }
...
UniValue listunspent(const UniValue& params, bool fHelp)
{
    ...
    BOOST_FOREACH (const COutput& out, vecOutputs) {
        ...
        entry.push_back(make_pair("txid", out.tx->GetHash().GetHex()));
        entry.push_back(make_pair("vout", out.i));
        CTxDestination address;
        if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, address)) {
            entry.push_back(make_pair("address", EncodeDestination(address)));
            if (pwalletMain->mapAddressBook.count(address))
                entry.push_back(make_pair("account", pwalletMain->mapAddressBook[address].name));
        }
        entry.push_back(make_pair("scriptPubKey", HexStr(pk.begin(), pk.end())));
        if (pk.IsPayToScriptHash()) {
            ...
            if (ExtractDestination(pk, address)) {
                ...
                if (pwalletMain->GetCScript(hash, redeemScript))
                    entry.push_back(make_pair("redeemScript", HexStr(redeemScript.begin(), redeemScript.end())));
            }
        }
        entry.push_back(make_pair("amount", ValueFromAmount(nValue)));
        entry.push_back(make_pair("confirmations", out.nDepth));
        entry.push_back(make_pair("spendable", out.fSpendable));
...
UniValue decodescript(const UniValue& params, bool fHelp)
{
    ...
    r.push_back(make_pair("p2sh", EncodeDestination(CTxDestination(CScriptID(script)))));
    return r;
}
...
{
    UniValue entry(UniValue::VOBJ);
    entry.push_back(make_pair("txid", txin.prevout.hash.ToString()));
    entry.push_back(make_pair("vout", (uint64_t)txin.prevout.n));
    entry.push_back(make_pair("scriptSig", HexStr(txin.scriptSig.begin(), txin.scriptSig.end())));
    entry.push_back(make_pair("sequence", (uint64_t)txin.nSequence));
    entry.push_back(make_pair("error", strMessage));
...
UniValue signrawtransaction(const UniValue& params, bool fHelp)
{
    ...
    UniValue result(UniValue::VOBJ);
    result.push_back(make_pair("hex", EncodeHexTx(mergedTx, PROTOCOL_VERSION)));
    result.push_back(make_pair("complete", fComplete));
    if (!vErrors.empty()) {
        result.push_back(make_pair("errors", vErrors));
    }
