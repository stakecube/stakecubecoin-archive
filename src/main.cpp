...
#if defined(NDEBUG)
#error "StakeCubeCore cannot be compiled without assertions."
#endif
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
        ...
        if (!IsBlockPayeeValid(block.vtx[1], pindex->nHeight)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    if (pindexPrev != NULL) {
        ...
        if (nHeight != 0 && !IsInitialBlockDownload()) {
            if (!IsBlockPayeeValid(block.vtx[1], nHeight)) {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv& inv = *it;
        {
            ...
            if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
                ...
                // Do not send blocks which are not-validated
    ...
    if (strCommand == "version") {
        ...
        if (strCommand == "tx") {
            ...
            if (pmn != NULL) {
                ...
                if (!obfuScationSigner.VerifyMessage(pmn->pubkey, vchSig, strMessage, errorMessage)) {
