...
int nAnonymizeStakecubeAmount = 1000;
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "stakecube" is a composite category enabling all StakeCubeCore-related debug output
            if (ptrCategory->count(string("stakecube"))) {
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty stakecube.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override stakecube.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "stakecubed.pid"));
