...
CzSCCTracker::CzSCCTracker(std::string strWalletFile)
{
...
CzSCCTracker::~CzSCCTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSCCTracker::Init()
{
...
bool CzSCCTracker::Archive(CMintMeta& meta)
{
...
bool CzSCCTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSCCTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSCCTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSCCTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzSCCTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzSCCTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzSCCTracker::GetSerialHashes()
{
...
CAmount CzSCCTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSCCTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSCCTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSCCTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSCCTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSCCTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSCCTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSCCTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSCCTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSCCTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSCCTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzSCCWallet* zSCCWallet)
{
    bool iszSCCWalletInitialized = (NULL != zSCCWallet);
    CMintMeta meta;
    ...
    if (! iszSCCWalletInitialized)
        zSCCWallet = new CzSCCWallet(strWalletFile);
    meta.isSeedCorrect = zSCCWallet->CheckSeed(dMint);
    if (! iszSCCWalletInitialized)
        delete zSCCWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzSCCTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSCCTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSCCTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSCCTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSCCTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSCCTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzSCCWallet* zSCCWallet = new CzSCCWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zSCCWallet);
        }
        delete zSCCWallet;
