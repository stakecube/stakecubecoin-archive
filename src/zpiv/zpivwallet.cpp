...
CzSCCWallet::CzSCCWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zscc seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZPIVSeed_deprecated(seed)) {
            ...
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZPIVSeed_deprecated()) {
                    LogPrintf("%s: Updated zSCC seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zscc seed\n", __func__);
                }
            }
        }
    }
...
bool CzSCCWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
...
void CzSCCWallet::Lock()
{
    seedMaster = 0;
}

void CzSCCWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSCCWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
...
bool CzSCCWallet::LoadMintPoolFromDB()
{
...
void CzSCCWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSCCWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSCCWallet::SyncWithChain(bool fGenerateMintPool)
{
...
bool CzSCCWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
...
void CzSCCWallet::SeedToZPIV(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSCCWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSCCWallet::GenerateDeterministicZPIV(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSCCWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSCCWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzSCCWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
