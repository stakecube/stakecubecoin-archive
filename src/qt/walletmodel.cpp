...
void WalletModel::pollBalanceChanged()
{
    ...
    if (fForceCheckBalanceChanged || chainHeight != cachedNumBlocks || cachedTxLocks != nCompleteTXLocks) {
        fForceCheckBalanceChanged = false;
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered stakecubecoin address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal stakecubecoin:URI (stakecubecoin:XyZ...?message=example)
            {
