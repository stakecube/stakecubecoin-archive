...
PrivacyWidget::PrivacyWidget(SCCGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zSCC anonymizes your SCC by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zSCC or convert back to SCC"));
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zSCC");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zSCC");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zSCC");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zSCC");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zSCC");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zSCC");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zSCC");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zSCC");
    ...
    ui->btnTotalzSCC->setTitleClassAndText("btn-title-grey", "Total 0 zSCC");
    ui->btnTotalzSCC->setSubTitleClassAndText("text-subtitle", "Show denominations of zSCC owned.");
    ui->btnTotalzSCC->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select SCC outputs to mint into zSCC.");
    ...
    connect(ui->btnTotalzSCC, SIGNAL(clicked()), this, SLOT(onTotalZpivClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zSCC");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of SCC to mint into zSCC"));
    }else{
        btnText = tr("Convert back to SCC");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zSCC to convert back into SCC"));
    }
...
void PrivacyWidget::onTotalZpivClicked(){
    ...
    if(!isVisible){
        ...
        ui->btnTotalzSCC->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzSCC->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(true) {
        warn(tr("Zerocoin"), tr("zSCC is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zSCC, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zSCC").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zSCC minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZpiv(
        ...
        // Spend succeed
        inform(tr("zSCC converted back to SCC"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zSCC </b>";
    ...
    ui->btnTotalzSCC->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
