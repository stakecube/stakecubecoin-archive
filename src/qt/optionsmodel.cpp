...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset){
    ...
    if (settings.contains("nAnonymizeSccAmount") || reset)
        SoftSetArg("-anonymizesccamount", settings.value("nAnonymizeSccAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in scc.cpp during shotdown to also remove the window positions
