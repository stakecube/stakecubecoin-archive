...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SCC address (e.g. %1)").arg("D7VFR83SQbiezrW72hjcWJtcfip5krte2Z"));
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient* out)
{
    // Convert scc:// to scc:
    //
    //    Cannot handle this later, because scc:// will cause Qt to see the part after // as host,
...
bool openConfigfile()
{
    ...
    /* Open scc.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SCC.lnk";
}
...
{
    return GetAutostartDir() / "scc.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a scc.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the scc app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add scc app to startup item list
