...
static int makedir(const char *newdir)
{
    ...
    if (buffer == NULL)
    {
        printf_err("Error allocating memory\n");
    ...
    while (1)
    {
        ...
        *p = 0;
        ...
        {
            snprintf(errbuf, ERRORBUFSIZE, "couldn't create directory %s (%d)\n", buffer, errno);
            printf_err(errbuf);
            free(buffer);
            return 0;
        }
...
static void display_zpos64(ZPOS64_T n, int size_char)
{
    ...
    snprintf(errbuf, ERRORBUFSIZE, "%s",&number[pos_string]);
    printf_err(errbuf);
}
...
static int do_list(unzFile uf)
{
    ...
    if (err != UNZ_OK)
    {
        snprintf(errbuf, ERRORBUFSIZE, "error %d with zipfile in unzGoToFirstFile\n", err);
        printf_err(errbuf);
        return 1;
    }

    printf_err("  Length  Method     Size Ratio   Date    Time   CRC-32     Name\n");
    printf_err("  ------  ------     ---- -----   ----    ----   ------     ----\n");
    ...
    do
    {
        ...
        if (err != UNZ_OK)
        {
            snprintf(errbuf, ERRORBUFSIZE, "error %d with zipfile in unzGetCurrentFileInfo\n", err);
            printf_err(errbuf);
            break;
        }
        ...
        snprintf(errbuf, ERRORBUFSIZE, "  %6s%c", string_method, charCrypt);
        printf_err(errbuf);

        display_zpos64(file_info.compressed_size, 7);
        snprintf(errbuf, ERRORBUFSIZE, " %3lu%%  %2.2lu-%2.2lu-%2.2lu  %2.2lu:%2.2lu  %8.8lx   %s\n", ratio,
    ...
    if (err != UNZ_END_OF_LIST_OF_FILE && err != UNZ_OK) {
        snprintf(errbuf, ERRORBUFSIZE, "error %d with zipfile in unzGoToNextFile\n", err);
        printf_err(errbuf);
        return err;
    }
...
static int do_extract_currentfile(unzFile uf, int opt_extract_without_path, int* popt_overwrite, const char *password)
{
    unz_file_info64 file_info = {0};
    FILE* fout = NULL;
    ...
    if (*filename_withoutpath == 0)
    {
        ...
        {
            snprintf(errbuf, ERRORBUFSIZE, "creating directory: %s\n", filename_inzip);
            printf_err(errbuf);
    ...
    if (err != UNZ_OK) {
        snprintf(errbuf, ERRORBUFSIZE, "error %d with zipfile in unzOpenCurrentFilePassword\n", err);
        printf_err(errbuf);
    }
    ...
    /* Determine if the file should be overwritten or not */
    if ((err == UNZ_OK) && (check_file_exists(write_filename)))
    {
        ...
        if (0 == *popt_overwrite)
            return 1;
        else if (2 == *popt_overwrite)
            skip = 1;
        else
            assert(1 == *popt_overwrite);
    }
    ...
    if (fout != NULL)
    {
        snprintf(errbuf, ERRORBUFSIZE, " extracting: %s\n", write_filename);
        printf_err(errbuf);
        ...
        do
        {
            ...
            if (err < 0)
            {
                snprintf(errbuf, ERRORBUFSIZE, "error %d with zipfile in unzReadCurrentFile\n", err);
                printf_err(errbuf);
                break;
            }
            if (err == 0)
            ...
            {
                snprintf(errbuf, ERRORBUFSIZE, "error %d in writing extracted file\n", errno);
                printf_err(errbuf);
                err = UNZ_ERRNO;
                break;
            }
    ...
    if (errclose != UNZ_OK) {
        snprintf(errbuf, ERRORBUFSIZE, "error %d with zipfile in unzCloseCurrentFile\n", errclose);
        printf_err(errbuf);
    }
...
static int do_extract_all(unzFile uf, int opt_extract_without_path, int opt_overwrite, const char *password)
{
    char errbuf[ERRORBUFSIZE] = {0};
    int err = unzGoToFirstFile(uf);
    if (err != UNZ_OK)
...
static int do_extract_onefile(unzFile uf, const char* filename, int opt_extract_without_path, int opt_overwrite,
    const char* password)
{
    char errbuf[ERRORBUFSIZE] = {0};
    if (unzLocateFile(uf, filename, NULL) != UNZ_OK)
    {
        snprintf(errbuf, ERRORBUFSIZE, "file %s not found in the zipfile\n", filename);
        printf_err(errbuf);
        return 2;
    }
    else
...
extern int ZEXPORT main_unzip(int argc, const char *argv[], error_callback fptr)
{
    ...
    if (uf == NULL)
    {
        snprintf(errbuf, ERRORBUFSIZE, "Cannot open %s\n", zipfilename);
        printf_err(errbuf);
        return 1;
    }

    snprintf(errbuf, ERRORBUFSIZE, "%s opened\n", zipfilename);
    printf_err(errbuf);
    ...
    else if (opt_do_extract == 1)
    {
        ...
        {
            snprintf(errbuf, ERRORBUFSIZE, "Error changing into %s, aborting\n", dirname);
            printf_err(errbuf);
            return -1;
        }
